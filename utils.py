...
class SimpleDownSampler(torch.utils.data.sampler.Sampler):
    ...
    def __init__(self, dataset, indices=None, num_samples=None):
        ...
        up_sampled_ids = []
        min_count = min(label_to_count)
        for label, count in enumerate(label_to_count):
            curr_ids = label_to_ids[label]
            random.shuffle(curr_ids)
            up_sampled_ids.extend(curr_ids[:min_count])
